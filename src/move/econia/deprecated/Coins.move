...
module Econia::Coins {
    ...
    struct EconiaHoldings has key {
        apt: Coin<APT>,
        usd: Coin<USD>
    }

    // Publish an empty EconiaHoldings resource under an account address
    // Must be called before transferring to the account
    public(script) fun publish_econia_holdings(
        account: signer // Account to publish under
    ) {
        let empty_apt = Coin<APT>{subunits: 0};
        let empty_usd = Coin<USD>{subunits: 0};
        assert!(
            !exists<EconiaHoldings>(Signer::address_of(&account)),
            E_ALREADY_HAS_HOLDINGS
        );
        move_to(&account, EconiaHoldings{apt: empty_apt, usd: empty_usd});
    }
    ...
    public fun balance_of(
        ...
        u64, // APT subunits
        u64 // USD subunits
    ) acquires EconiaHoldings {
        let apt_subunits = borrow_global<EconiaHoldings>(owner).apt.subunits;
        let usd_subunits = borrow_global<EconiaHoldings>(owner).usd.subunits;
    ...
    fun deposit(
        ...
        let apt_balance_ref_mut =
            &mut borrow_global_mut<EconiaHoldings>(addr).apt.subunits;
        let Coin{subunits: apt_subunits} = apt_coin;
        *apt_balance_ref_mut = apt_balance + apt_subunits;
        let usd_balance_ref_mut =
            &mut borrow_global_mut<EconiaHoldings>(addr).usd.subunits;
        let Coin{subunits: usd_subunits} = usd_coin;
        *usd_balance_ref_mut = usd_balance + usd_subunits;
    }

    // Mint APT and USD amount to the Econia module address
    // Can only be invoked by the Econia module address
    public(script) fun mint(
        ...
        assert!(account_addr == @Econia, E_NOT_ULTIMA_SIGNER);
        deposit(
            account_addr,
    ...
    #[test]
    #[expected_failure]
    fun balance_of_dne() acquires EconiaHoldings {
        balance_of(@TestUser1);
    }

    // Verify Econia address can mint circulating supply
    #[test(
        user1_tx1 = @TestUser1,
        user1_tx2 = @TestUser1,
        user1_tx3 = @TestUser1,
    ...
    public(script) fun mint_and_transfer(
        user1_tx1: signer, // Non-Econia address, signer for 1st tx
        user1_tx2: signer, // Non-Econia address, signer for 2nd tx
        user1_tx3: signer, // Non-Econia address, signer for 3rd tx
        user2_tx1: signer, // Another non-Econia address, for 1st tx
        user2_tx2: signer, // Another non-Econia address, for 2nd tx
        econia_tx1: signer, // Econia address, signer for 1st tx
        econia_tx2: signer, // Econia address, signer for 2nd tx
        econia_tx3: signer, // Econia address, signer for 3rd tx
    ) acquires EconiaHoldings {
        // Initialize addresses to empty holdings
        publish_econia_holdings(user1_tx1);
        publish_econia_holdings(user2_tx1);
        publish_econia_holdings(econia_tx1);

        // Get addresses
        let user1_addr = Signer::address_of(&user1_tx2);
        let user2_addr = Signer::address_of(&user2_tx2);
        let ult_addr = Signer::address_of(&econia_tx2);

        // Mint to Econia address and transfer to user1
        mint(econia_tx2, 250, 1275);
        transfer(econia_tx3, user1_addr, 100, 1000);
    ...
    // Verify Econia address can mint APT and USD to itself
    #[test(
        econia_signer_tx1 = @Econia,
        econia_signer_tx2 = @Econia
    )]
    public(script) fun mint_econia(
        econia_signer_tx1: signer, // Econia account signer, 1st tx
        econia_signer_tx2: signer // Econia account signer, 2nd tx
    ) acquires EconiaHoldings {
        publish_econia_holdings(econia_signer_tx1);
        let econia_addr = Signer::address_of(&econia_signer_tx2);
        mint(econia_signer_tx2, 123, 456);
        let(apt_subunits, usd_subunits) = balance_of(econia_addr);
        // Assert correct balance minted
        assert!(apt_subunits == 123, E_WRONG_MINT_BALANCE);
        assert!(usd_subunits == 456, E_WRONG_MINT_BALANCE);
    }

    // Verify arbitrary user can properly initialize EconiaHoldings
    ...
    public(script) fun publish_balance_has_zero(
        ...
        publish_econia_holdings(account);
    ...
    // Verify error asserted when EconiaHoldings published twice
    #[test(
    ...
    public(script) fun publish_holdings_twice(
        ...
        publish_econia_holdings(account_tx1);
        publish_econia_holdings(account_tx2);
    }
    ...
    public(script) fun self_transfer (
        econia_tx1: signer, // Econia account signer, 1st tx
        econia_tx2: signer, // Econia account signer, 2nd tx
        econia_tx3: signer // Econia account signer, 3rd tx
    ) acquires EconiaHoldings {
        publish_econia_holdings(econia_tx1);
        mint(econia_tx2, 123, 456);
        let econia_tx3_addr = Signer::address_of(&econia_tx3);
        transfer(econia_tx3, econia_tx3_addr, 5, 10);
    }

    // Verify unauthorized user cannot mint coins
    #[test(not_econia = @TestUser1)]
    #[expected_failure(abort_code = 2)]
    public(script) fun unauthorized_minter(
        not_econia: signer // Signer that is not Econia account
    ) acquires EconiaHoldings {
        mint(not_econia, 0, 0);
    }
    ...
    #[test]
    #[expected_failure]
    fun withdraw_dne() acquires EconiaHoldings {
