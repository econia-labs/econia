...
module Econia::User {
    ...
    /// Initialize a user with `Econia::Orders::OO` and `CC` for market
    ...
    public(script) fun init_user<B, Q, E>(
        ...
        assert!(!exists<CC<B, Q, E>>(user_addr), E_C_C_EXISTS);
...
*/
    ...
    public(script) fun init_o_i_c_c_failure_oicc_exists(
        econia: &signer
    ) {
    ...
    #[expected_failure(abort_code = 0)]
    /// Verify failure for user already having collateral container
    public(script) fun init_user_failure_has_c_c(
        econia: &signer,
        user: &signer
    ) acquires OICC {
        r_r_t_m(econia); // Register test market
        init_c_c<BCT, QCT, E0>(user); // Register collateral container
    ...
    public(script) fun init_user_success(
        ...
        let user_addr = s_a_o(user); // Get user address
        // Borrow immutable reference to collateral container
        let c_c = borrow_global<CC<BCT, QCT, E0>>(user_addr);
        // Assert no base coins or quote coins in collateral container
        assert!(c_v(&c_c.b_c) == 0 && c_v(&c_c.q_c) == 0, 0);
        // Assert no base coins or quote coins marked available
        assert!(c_c.b_a == 0 && c_c.q_a == 0, 1);
