...
CREATE VIEW api.markets AS
    ...
    ON
        base.address = COALESCE(m.base_account_address, '')
    AND
        base.module = COALESCE(m.base_module_name, '')
    AND
        base.struct = COALESCE(m.base_struct_name, '')
    LEFT JOIN
        aggregator.coins AS quote
    ON
        quote.address = COALESCE(m.base_account_address, '')
    AND
        quote.module = COALESCE(m.base_module_name, '')
    AND
        quote.struct = COALESCE(m.base_struct_name, '')
    LEFT JOIN
        first_fills AS f
    ON
