-- Your SQL goes here
CREATE MATERIALIZED VIEW aggregator.markets_24h_data2 AS
    WITH fills AS(
        SELECT
            *
        FROM
    ...
    SELECT
        ...
        size_to_base_indivisible_subunits(fills.market_id, SUM(fills.size)) AS base_volume_24h,
        SUM(size_and_price_to_quote_indivisible_subunits(fills.market_id, fills.size, fills.price)) AS quote_volume_24h
    FROM
        fills
    GROUP BY
        market_id;
...
SELECT
  ...
  (SELECT SUM(volume) FROM api.candlesticks AS c WHERE c.market_id = m.market_id AND resolution = 86400) AS quote_volume
FROM
  ...
  LEFT JOIN aggregator.markets_24h_data2 AS v ON v.market_id = m.market_id;
