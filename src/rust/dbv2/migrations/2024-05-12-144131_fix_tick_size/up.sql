...
CREATE OR REPLACE FUNCTION api.quote_indivisible_subunits_to_nominal (market_id numeric, subunits numeric) RETURNS NUMERIC AS $$
    SELECT subunits / POW(10::numeric,quote_decimals::numeric) FROM api.markets WHERE markets.market_id = $1;
$$ LANGUAGE SQL;


CREATE OR REPLACE FUNCTION get_quote_volume_divisor_for_market(numeric) RETURNS NUMERIC IMMUTABLE AS $$
    SELECT POW(10::numeric,decimals::numeric) / tick_size
...
SELECT
    ...
    api.quote_indivisible_subunits_to_nominal(market_id, api.get_market_liquidity(market_id,200)) AS liquidity_in_quote
FROM
    api.markets;
