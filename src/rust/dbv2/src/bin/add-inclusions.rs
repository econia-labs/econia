...
fn main() {
    let inclusions_json = fs::read_to_string("./competition-additional-inclusions.json").expect("No config");
    ...
    connection.transaction(|conn| {
        ...
        for inclusion in &inclusions {
            let size = diesel::delete(
                competition_exclusion_list::table
                    .filter(
                        competition_exclusion_list::user.eq(&inclusion.user)
                            .and(competition_exclusion_list::competition_id.eq(inclusion.competition_id)),
                    ),
            )
            .execute(conn)
