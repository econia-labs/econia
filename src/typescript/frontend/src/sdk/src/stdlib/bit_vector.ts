...
export function is_index_set_(
  ...
  if (
    !$.copy(bit_index).lt(
      Vector.length_(bitvector.bit_field, $c, [AtomicTypeTag.Bool])
    )
  ) {
    throw $.abortCode($.copy(EINDEX));
  }
  return $.copy(
    Vector.borrow_(bitvector.bit_field, $.copy(bit_index), $c, [
      AtomicTypeTag.Bool,
    ])
  );
}

export function length_(bitvector: BitVector, $c: AptosDataCache): U64 {
...
export function new___(length: U64, $c: AptosDataCache): BitVector {
  ...
  return new BitVector(
    { length: $.copy(length), bit_field: $.copy(bit_field) },
    new SimpleStructTag(BitVector)
  );
}
...
export function set_(
  ...
  x = Vector.borrow_mut_(bitvector.bit_field, $.copy(bit_index), $c, [
    AtomicTypeTag.Bool,
  ]);
  $.set(x, true);
  return;
...
export function shift_left_(
  ...
  if ($.copy(amount).ge($.copy(bitvector.length))) {
    ...
    while ($.copy(i).lt($.copy(len))) {
      {
        elem = Vector.borrow_mut_(bitvector.bit_field, $.copy(i), $c, [
          AtomicTypeTag.Bool,
        ]);
        $.set(elem, false);
        i = $.copy(i).add(u64("1"));
