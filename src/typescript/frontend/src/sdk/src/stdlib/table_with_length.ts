...
export class TableWithLength {
  ...
  static structName = "TableWithLength";
...
export function new___(
  ...
  return new TableWithLength(
    { inner: Table.new___($c, [$p[0], $p[1]]), length: u64("0") },
    new SimpleStructTag(TableWithLength, [$p[0], $p[1]])
  );
}
...
export function loadParsers(repo: AptosParserRepo) {
  repo.addParser(
    "0x1::table_with_length::TableWithLength",
    TableWithLength.TableWithLengthParser
  );
}
export class App {
  constructor(
