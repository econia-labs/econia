...
export function is_module_allowed_ (
  $c: AptosDataCache,
): boolean {
  let publish_option;
...
export function is_script_allowed_ (
  script_hash: U8[],
  $c: AptosDataCache,
): boolean {
  let temp$1, publish_option;
  if (Std.Vector.is_empty_(script_hash, $c, [AtomicTypeTag.U8])) {
    return true;
  }
  else{
  }
  publish_option = $c.borrow_global<TransactionPublishingOption>(new StructTag(new HexString("0x1"), "transaction_publishing_option", "TransactionPublishingOption", []), new HexString("0x1"));
  if (Std.Vector.is_empty_(publish_option.script_allow_list, $c, [new VectorTag(AtomicTypeTag.U8)])) {
    temp$1 = true;
  }
  else{
    temp$1 = Std.Vector.contains_(publish_option.script_allow_list, script_hash, $c, [new VectorTag(AtomicTypeTag.U8)]);
  }
  return temp$1;
}

export function set_module_publishing_allowed_ (
  account: HexString,
  is_allowed: boolean,
  $c: AptosDataCache,
): void {
