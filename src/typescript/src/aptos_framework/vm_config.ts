...
export const moduleName = "vm_config";
...
export class GasSchedule 
{
  ...
  { name: "gas_constants", typeTag: new StructTag(new HexString("0x1"), "vm_config", "GasConstants", []) }];
...
export class VMConfig 
{
  ...
  static fields: FieldDeclType[] = [
  { name: "gas_schedule", typeTag: new StructTag(new HexString("0x1"), "vm_config", "GasSchedule", []) }];
...
export function set_gas_constants_ (
  account: HexString,
  global_memory_per_byte_cost: U64,
  global_memory_per_byte_write_cost: U64,
  min_transaction_gas_units: U64,
  ...
  if (!($.copy(min_price_per_gas_unit)).le($.copy(max_price_per_gas_unit))) {
    throw $.abortCode(Std.Error.invalid_argument_(EGAS_CONSTANT_INCONSISTENCY, $c));
  }
...
export function buildPayload_set_gas_constants (
  ...
  return $.buildPayload(
    "0x1::vm_config::set_gas_constants",
    typeParamStrings,
    [
