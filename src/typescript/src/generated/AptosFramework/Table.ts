...
export function add_box$ (
  ...
  return $.AptosFramework_Table_add_box(table, key, val, $c, [$p[0], $p[1], $p[2]]);
...
export function borrow_box$ (
  ...
  return $.AptosFramework_Table_borrow_box(table, key, $c, [$p[0], $p[1], $p[2]]);
...
export function borrow_box_mut$ (
  ...
  return $.AptosFramework_Table_borrow_box_mut(table, key, $c, [$p[0], $p[1], $p[2]]);
...
export function contains_box$ (
  ...
  return $.AptosFramework_Table_contains_box(table, key, $c, [$p[0], $p[1], $p[2]]);
...
export function destroy_empty$ (
  ...
  if (!$.copy(table.length).eq(u64("0"))) {
    throw $.abortCode(Std.Errors.invalid_state$(ENOT_EMPTY, $c));
  }
...
export function drop_unchecked_box$ (
  ...
  return $.AptosFramework_Table_drop_unchecked_box(table, $c, [$p[0], $p[1], $p[2]]);
...
export function new__$ (
  ...
  return new Table({ handle: new_table_handle$($c), length: u64("0") }, new StructTag(new HexString("0x1"), "Table", "Table", [$p[0], $p[1]]));
}

export function new_table_handle$ (
  $c: AptosDataCache,
): U128 {
  return $.AptosFramework_Table_new_table_handle($c);
...
export function remove_box$ (
  ...
  return $.AptosFramework_Table_remove_box(table, key, $c, [$p[0], $p[1], $p[2]]);
